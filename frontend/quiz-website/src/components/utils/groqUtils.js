...
async function generateQuestionsFromText(text) {
  ...
  const chatCompletion = await groq.chat.completions.create({
    messages: [
      ...
      {
        role: "user",
        content: `Generate a set of questions from the following text: ${text}. Ensure that the correct answer option is randomized among the options. The output should be a JSON object that conforms to the provided schema.`,
      },
    ],
